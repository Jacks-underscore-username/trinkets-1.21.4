...
public class GiveItem extends Task {
    ...
    static {
        ...
        validItems.put(Items.ENDER_PEARL, new validItem(4, 32, "I need a faster way of traveling, could you bring me %s ender pearlS?"));
        ...
        validItems.put(Items.OMINOUS_TRIAL_KEY, new validItem(5, 5, "I've heard about some new rare item, could you bring me %s ominous trial keyS?"));
        validItems.put(Items.TRIAL_KEY, new validItem(5, 10, "I've heard about some new rare item, could you bring me %s trial keyS?"));
    ...
    public DialogPage getPage() {
        ...
        String text = validItems.get(cost.getItem()).message.replaceAll("%s", String.valueOf(cost.getCount())).toLowerCase();
        if (cost.getCount() == 1)
        ...
        DialogPage.Callback callback = (subPlayer, inventory) -> {
            if (inventory.getStack(0).isOf(cost.getItem()) && inventory.getStack(0).getCount() >= cost.getCount()) {
                ...
                QuestManager.finishTask(subPlayer.server, this.entry, entry.value * entry.max / cost.getCount());
            }
        };
