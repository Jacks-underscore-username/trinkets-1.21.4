...
public class Config {
    ...
    @SerializedName("// The maximum number of trinkets a player can have, if they exceed this limit then all their trinkets lose function until the player has less then the limit, set to zero to remove the limit")
    public final String _comment_max_player_trinkets = "(Default: 0)";
    @SerializedName("Max trinkets per player")
    public int max_player_trinkets = 1;

    @SerializedName("// The mode used to determine if a player has too many trinkets, only matters if 'Max trinkets per player' is > 0")
    public final String _comment_player_limit_mode = "(Default: 2)";
    @SerializedName("// 1: Only counts the trinkets when the player uses them")
    public final String _comment_player_limit_mode_0 = "";
    @SerializedName("// 2: All trinkets are now 'claimed' when picked up until another player picks it up or it is destroyed, counts all trinkets currently claimed by the player")
    public final String _comment_player_limit_mode_2 = "";
    public int player_limit_mode = 2;

    @SerializedName("// Delays trinket disablement for modes 1 and 2 until the condition has been met for X seconds, this is to prevent a player throwing a trinket at someone else and disabling their trinkets")
    ...
    @SerializedName("// Delays trinket enablement until the player has below the limit of trinkets for X seconds")
